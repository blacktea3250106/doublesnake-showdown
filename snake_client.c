...
void main(void)
{
	...
	int server_n,result = 0;

	int port=8877;
	client_addr.sin_family = AF_INET;
	client_addr.sin_addr.s_addr =  INADDR_ANY; 
	client_addr.sin_port = htons(port);
	res = connect(socket_desc, (struct sockaddr*)&client_addr, sizeof(client_addr));
...
void Init_Disp()
{
    ...
    mvaddstr(2, 70, "Snake 2: Press '1' to speed up, '2' to speed down");
...
void gameover(int snake,int n)
{
    ...
    if(snake == 1){
        s1 = 1;
        s2 = 2;
		write(socket_desc,&s1,1);
		write(socket_desc,&length1,1);
    }else{
        s1 = 2;
        s2 = 1;
		write(socket_desc,&s1,1);
		write(socket_desc,&length2,1);
    }
	switch(n)
	{
		case 1:
            mvprintw(LINES/2, COLS/3 - 4,"snake%d WIN!!,snake%d crash the wall,~~Score To Server~~\n",s1,s2);
			break;
		case 2:
            mvprintw(LINES/2, COLS/3 - 4,"snake%d WIN!!,snake%d crash yourself,~~Score To Server~~\n",s1,s2);
			break;
        case 3:
            mvprintw(LINES/2, COLS/3 - 4,"snake%d WIN!!,snake%d crash the bmob,~~Score To Server~~\n",s1,s2);
			break;
        case 4:
            mvprintw(LINES/2, COLS/3 - 4,"snake%d WIN!!,snake%d crash the snake%d's body,~~Score To Server~~\n",s1,s2,s1);
		default:
			break;
	}
    write(socket_desc,&snake,1);
